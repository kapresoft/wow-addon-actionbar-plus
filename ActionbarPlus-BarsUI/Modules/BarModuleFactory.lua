...
local function BarModuleProtoMethods()
  ...
  function bm:OnEnable()
    print('BarModuleFactory', 'OnEnable')
    t('OnEnable')
  ...
  function bm:OnDisable()
    print('BarModuleFactory', 'OnDisable')
    t('OnDisable')
...
local function PropsAndMethods()
  ...
  function o:New(barFrame)
    assertsafe(type(barFrame) == 'table', 'New(barFrame):: {barFrame} should be a frame.')
    local core = ns:a()
    ...
    m.barFrame = barFrame
    m.index = w.index
    t('New', 'enabled=', m:c().enabled)
    if m:c().enabled then m:Enable()
    else m:Disable() end
    
    C_Timer.After(1, function()
      t(('New:: %s created; enabled=%s'):format(m:GetName(), tostring(m:IsEnabled())))
    end)
    _G[name] = m
