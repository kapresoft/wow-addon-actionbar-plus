...
local String, Assert, Table = O.String, O.Assert, O.Table
...
local IsBlank, IsNil, IsEmptyTable = String.IsBlank, Assert.IsNil, Table.IsEmpty
...
local function PropsAndMethods(o)
    ...
    function o:New(btnWidget)
        return ns:K():CreateAndInitFromMixin(O.ButtonDataMixin, btnWidget)
    end

    ---@param btnWidget ButtonUIWidget
    function o:Init(btnWidget)
        assert(btnWidget, "ButtonUIWidget is required")
        self.config = P:GetButtonDataByIndex(btnWidget.frameIndex, btnWidget.index)
        self.widget = btnWidget
    end

    function o:GetConfig() return self.config end

    function o:IsEmpty()
        local type = self.config.type
        if IsBlank(type) then return true end
