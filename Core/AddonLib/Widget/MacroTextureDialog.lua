...
local TEXTURE_EMPTY, ANCHOR_TOPLEFT = TEXTURE_EMPTY, ANCHOR_TOPLEFT
local LogFactory = ABP_LogFactory
local GameTooltip = GameTooltip
local ACELIB, MC = AceLibFactory, MacroIconCategories
local ART_TEXTURES, ConfigureFrameToCloseOnEscapeKey = ART_TEXTURES, ConfigureFrameToCloseOnEscapeKey
local GetMacroItemIcons = GetMacroItemIcons

local D = {}
LogFactory:EmbedLogger(D, '::TextureDialog')
ABP_MacroTextureDialog = D
...
function D:CreateTexturePopupDialog()
    local iconSize = 50
...
function D:FetchCategoriesCache()
    if categoryCache or type(categoryCache) == 'table' then return end
    categoryCache = MC:GetCategoriesCache()
    self:log(10, 'Macro icon categories cache fetched')
end

function D:FetchMacroIcons()
    if macroIcons or type(macroIcons) == 'table' then return end
